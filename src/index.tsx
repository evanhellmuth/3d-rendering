...
export const fileViewerSTL: FC = () => {
  ...
  const [retoolAnnotations, setRetoolAnnotations] = Retool.useStateArray({
    name: 'annotations',
    label: 'Annotations'
  });
  ...
  // Initialize annotations only once when component mounts
  useEffect(() => {
    // Initialize annotations only once when component mounts
    if (retoolAnnotations && retoolAnnotations.length > 0) {
      // Convert the retool annotations format to a three.js-compatible format
      const localAnnotations = retoolAnnotations.map((anno, index) => ({
        id: Date.now() + index, // Generate unique IDs
        label: index + 1,
        text: anno.text,
        position: new THREE.Vector3(anno.position.x, anno.position.y, anno.position.z)
      }));
  ...
  const saveAnnotations = () => {
    // Convert your annotations array to the format you want to save
    ...
    setRetoolAnnotations(formattedAnnotations);

    // Tell parent app about the new annotations
